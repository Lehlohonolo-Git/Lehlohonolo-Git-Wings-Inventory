    ...
    const addOrUpdateProduct = (e) => {
        ...
        fetch(url, {
            ...
            .then(newProduct => {
                ...
                if (editIndex === -1) {
                    updatedProducts.push(newProduct); // Add new product
                } else {
                    updatedProducts[editIndex] = newProduct; // Update existing product
                    setEditIndex(-1);
                }
                setProducts(updatedProducts);
                setProduct({ name: '', desc: '', category: '', price: '', quantity: '' }); // Reset form
            })
    ...
    <tbody>
      {products.map((product, index) => (
        <tr key={index}>
          ...
          <td>
            <button onClick={() => sellProduct(index)} disabled={product.quantity <= 0}>
              Sell
            </button>
